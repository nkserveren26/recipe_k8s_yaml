...
## Argo CD
当初は手動で Kubernetes 上にアプリケーションをデプロイするやり方で運用していたが、アプリを更新するたびに、
対応する Deployment マニフェストを修正する必要があり、修正後のマニフェストを Kubernetes クラスター上で適用する必要があった。
以下の手順でアプリのデプロイを行っており、手数を要することもあり、オペミスのリスクがあった。
- Deployment マニフェスト更新
- 更新したマニフェストファイルを Kubernetes クラスター上に配置
- 更新したマニフェストファイルの内容で Deployment を手動更新 (kubectl apply)

こうした課題を解決するために、Argo CD による自動デプロイの仕組みを導入した (CD 導入)。
Argo CD でマニフェストを配置しているリポジトリを監視し、マニフェストが更新されたら、
自動で Kubernetes クラスターへ反映する仕組みを導入。
これにより、手動によるオペミスを解消。
また、単一のリポジトリ上でマニフェストを管理することで、Kubernetes リソースを一元管理し、
現在、Kubernetes クラスターにどの状態のマニフェストが反映されているかを見える化した。
