...
## Cloudflare
### 課題
Cloudflare 導入前は Kubernetes クラスター上で NodePort リソースを使用して、IP アドレスを直接指定してアプリにアクセスしていた。  
この方式では HTTP での通信であり、家庭のプライベートネットワーク内のアクセスとはいえセキュリティ的によろしくない状態であった。
また、家のプライベートネットワーク内でのみアプリにアクセスできる状態であり、今後このネットワーク以外の環境からアクセスしたい場合（家の外からアクセスしたい場合）に不都合となる可能性があった。

### 解決策
Cloudflare Tunnel を導入。

### 効果
- HTTPS 通信となり、セキュアな通信でアプリにアクセスできるようになった
- 家の外からもアプリにアクセス可能になった
- Kubernetes クラスター基盤は引き続きインターネットからは見えない状態なので、セキュア
